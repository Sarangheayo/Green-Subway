 ...
 * 첫차/막차 조회
 */
export const fetchFirstLastByLine = createAsyncThunk(
  "stationDetail/fetchFirstLastByLine",
  async ({ line, dow = 1, start = 1, end = 50 }) => {
    ...
    const [up, down] = await Promise.all([
      axios.get(makeUrl(1)).then(r => r?.data?.SearchFirstAndLastTrainbyLineServiceNew?.row ?? []).catch(()=>[]),
      axios.get(makeUrl(2)).then(r => r?.data?.SearchFirstAndLastTrainbyLineServiceNew?.row ?? []).catch(()=>[]),
    ]);
